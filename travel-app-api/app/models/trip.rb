class Trip < ApplicationRecord
  ...
  def retreive_lng_lat_img_url
    search_url = "https://maps.googleapis.com/maps/api/place/details/json?reference=#{self.google_id}&key=#{Figaro.env.google_api_key}vM"
    ...
    img_search_url = "https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=#{self.lat},#{self.lng}&radius=50000&rankBy=prominence&key=#{Figaro.env.google_api_key}vM"
    ...
    self.img_url = "https://maps.googleapis.com/maps/api/place/photo?maxwidth=1600&photoreference=#{photo_ref}&key=#{Figaro.env.google_api_key}vM"
    self
  end
